...
export function CalibrationPhase() {
  const { calibrationTimeLeft, startCalibration, skipCalibration } =
    useTelemetryStore();
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center animate-in zoom-in-95 duration-700">
      ...
      <p className="text-ds-muted max-w-md mx-auto text-sm leading-relaxed">
        Estableciendo línea base de frecuencia cardíaca. Por favor, manténgase
        en reposo con el sensor MAX30102 colocado durante los próximos segundos
        antes de iniciar su viaje.
      </p>

      {/* --- BOTÓN DE DEV MODE (OCULTAR EN PRODUCCIÓN) --- */}
      <button
        onClick={skipCalibration}
        className="flex items-center gap-2 px-5 py-2.5 bg-white/5 hover:bg-white/10 border border-white/10 hover:border-ds-primary/50 text-ds-muted hover:text-white rounded-xl text-xs uppercase tracking-widest font-bold transition-all group"
      >
        <FastForward
          size={16}
          className="text-ds-primary group-hover:translate-x-1 transition-transform"
        />
